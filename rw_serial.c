...
int read_queue_cond(void *data)
{
	return Crowd_Empty(serializer, writers_crowd);
}

int write_queue_cond(void *data)
{
...
void *read_func(void *id)
{
	...
	Serial_Enqueue(serializer, waiting_q, &read_queue_cond, 0, NULL);
	Serial_Join_Crowd(serializer, readers_crowd,(void *) (&read_data), NULL);
...
void *write_func(void *id)
{
	...
	Serial_Enqueue(serializer, waiting_q, &write_queue_cond, 0, NULL);
	Serial_Join_Crowd(serializer, writers_crowd, &write_data, NULL);
	Serial_Exit(serializer);
